...
struct PageTableEntry {
    ...
    uint64_t address() const {
        if (huge_page()) return value & 0xFFFFFFFE00000ULL;
        return value & 0xFFFFFFFFF000ULL;
    }

    void set_present(bool x) {
        value = (value & ~1ULL) | (x ? 1ULL : 0);
    }
    void set_writable(bool x) {
        value = (value & ~(1ULL << 1)) | (x ? (1ULL << 1) : 0);
    }
    void set_user(bool x) {
        value = (value & ~(1ULL << 2)) | (x ? (1ULL << 2) : 0);
    }
    void set_pwt(bool x) {
        value = (value & ~(1ULL << 3)) | (x ? (1ULL << 3) : 0);
    }
    void set_pcd(bool x) {
        value = (value & ~(1ULL << 4)) | (x ? (1ULL << 4) : 0);
    }
    void set_accessed(bool x) {
        value = (value & ~(1ULL << 5)) | (x ? (1ULL << 5) : 0);
    }
    void set_dirty(bool x) {
        value = (value & ~(1ULL << 6)) | (x ? (1ULL << 6) : 0);
    }
    void set_huge_page(bool x) {
        value = (value & ~(1ULL << 7)) | (x ? (1ULL << 7) : 0);
    }
    void set_global(bool x) {
        value = (value & ~(1ULL << 8)) | (x ? (1ULL << 8) : 0);
    }
